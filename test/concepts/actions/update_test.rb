  ...
  class UpdateTest < ActionDispatch::IntegrationTest
    ...
    test 'Permission Deny : No Session' do
      e = assert_raises InvalidPermissions do
        Operation::Update.call(params: { id: actions(:list).id, name: 'This is name.' })
      end
      assert_equal ['Permissions is invalid'], JSON.parse(e.message)
    end

    test 'Update Data' do
      ctx = Operation::Update.call(params: { id: actions(:list).id, name: 'This is name.' }, current_user: @current_user, action: 'DUMMY_ACTION_ID')
      assert ctx.success?
